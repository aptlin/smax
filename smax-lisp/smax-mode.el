...
(defun smax-customize-user ()
  "Open smax/user/user.el. If it does not exist, copy the example
one and open it."
  (interactive)
...
(easy-menu-define my-menu smax-mode-map "My own menu"
  '("Smax"
    ...
    ("bibtex"
     ...
     ["  Open entry in Google Scholar" smax-bibtex-google-scholar (eq major-mode 'bibtex-mode)]
     ["  Open entry in Crossref" smax-bibtex-crossref (eq major-mode 'bibtex-mode)]
     ["  Open entry in Pubmed" smax-bibtex-pubmed (eq major-mode 'bibtex-mode)]
    ...
    ["Help with smax" smax-help t]
    ["Customize user.el" smax-customize-user t]))


(define-minor-mode smax-mode
  "Minor mode for smax

\\{smax-mode-map}"
  :global t
  :keymap smax-mode-map)

(provide 'smax-mode)

;;; smax-mode.el ends here
