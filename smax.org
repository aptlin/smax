...
Smax is an Emacs starterkit designed for people interested in reproducible research and publishing. Smax is just Emacs that has been configured extensively to make it act like we need it to for research documentation and publication. The notes below document some features we have built into smax.

* Org-mode
smax provides a lot of customization of org-mode. Here are some of the more important ones.

** Markup shortcuts
smax defines some shortcuts for org-mode markup. You can select text and type the shortcut to wrap the region in the markup, or if on a word, type the shortcut to wrap the word, or type the short cut to create the markup with the cursor inside it to keep typing.
...
The following examples show the shortcuts defined in smax for expansion. Put your cursor after the shortcut and press tab to expand them.
...
For numbered LaTeX equations, smax will make sure that the overlays have correct numbers on them.

*** Latex classes that smax knows about
...
Image rescaling in org-mode/Emacs used to require an Emacs with imagemagick support compiled in. This is a problem on Windows. Smax support image rescaling with an external imagemagick program called mogrify. You can rescale the appearance of an image in org-mode by adding an attribute like this above the figure.
...
smax defines these new speed commands that are active when the cursor is on the first character of a headline.
...
Smax provides some support for track changes and edit marks in org-mode.
...
smax provides some commands to:
...
Smax provides some support for highlighting, comment overlays, and edit marks.
...
Bibliographies in smax are stored in bibtex files. A bibtex file is a plain text file containing bibtex entries. Each entry describes an item. Here is a typical example.
...
[[https://github.com/Malabarba/emacs-google-this][google-this]] is installed in smax. You can use these commands to search for things from Emacs:
...
Pygments doesn't support ipython out of the box for some reason, which is a problem if you want to export your src block to LaTeX. smax fixes this for you and automatically installs this if you don't already have it.
...
#+RESULTS:
: The slowest run took 263.68 times longer than the fastest. This could mean that an intermediate result is being cached.
: 100000000 loops, best of 3: 9.6 ns per loop
:RESULTS:
42
:END:
...
smax provides the jupyter-hy src block to run hylang in src blocks. The required :session and :kernel headers are automatically provided.
...
It is a major challenge to keep up with the scientific literature. `elfeed' is the package we use in smax for that. It aggregates RSS feeds and provides a pretty easy way to consume them, capture them in to org-mode, search them, and do things with them. smax preconfigures elfeed with some python, and emacs feeds, and you can easily add new feeds:
...
You customize `smax-user-hotspot-commands' and `smax-user-hotspot-locations'.
...
#+BEGIN_SRC emacs-lisp
(setq smax-user-hotspot-commands
      '(("Mail" . (lambda ()
...
These are external packages that are included in smax and might be useful for you.
...
In particular, smax sets these keybindings:
...
smax provides a scientific notebook capability. Each "notebook" is actually collection of org-files in a "project". A project is the set of files in a directory that is under git version control. Each project should have a master file (the default is README.org, but you can customize `nb-master-file' to change this). The master file contains what ever you want, but typically it links to other documents in the project and provides an overview of the project.
...
smax defines these additional key bindings:
...
smax currently uses ivy extensively for completion, and enables `ivy-mode' and `counsel-mode'. The default matching behavior is `ivy--regex-ignore-order'.
...
`smax-help' will open this document.
...
smax also defines a link for man pages.
